...
/// Composition alone, for names typed or dropped locally: the same NFC that
/// replay's [`sanitize`] applies, so a decomposed name (macOS hands those
/// back) compares against, and is stored as, the row it will become.
