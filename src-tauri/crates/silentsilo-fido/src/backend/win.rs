...
pub fn complete_enrollment(challenge: &EnrollmentChallenge) -> Result<Enrollment, FidoError> {
    ...
    if !hmac_ok {
        ...
        // remove "iPhone, iPad, or Android device" from it. A phone reached over
        // hybrid hands back a passkey with no hmac-secret, so there is nothing to
        ...
        return Err(FidoError::EnrollmentFailed(if via_phone {
            "A passkey on a phone cannot unlock a silo. Scanning the QR code stores the \
             passkey on the phone, which does not offer the hmac-secret extension the silo \
             key is derived from. Plug a FIDO2 key into this computer, or use Windows Hello."
                .into()
        } else if platform {
