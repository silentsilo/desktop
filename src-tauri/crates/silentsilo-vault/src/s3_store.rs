...
/// last â€” the fastest way to have a family silo replaying a work silo's
/// operation log.
...
/// `None` means sync simply isn't set up, which is a normal state â€” the app
...
/// Disconnects sync. Best-effort: a missing entry isn't an error.
...
// â”€â”€ More than one target â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
pub struct BackupTarget {
    ...
    #[serde(default)]
    pub role: TargetRole,
    /// Space bought from SilentSilo, rather than storage the user already
    /// had. Recorded rather than guessed from the endpoint: an account holds
    /// one allocation, so the screen has to know not to offer selling a
    /// second one, and matching on a hostname would break the day the
    /// storage moves.
    #[serde(default)]
    pub hosted: bool,
    /// Reads how much of the bought space is in use. Issued at pairing and
    /// kept beside the storage credentials. Only ever used to show a
    /// figure, so an absent or refused token costs a line on a screen.
    #[serde(default)]
...
///
/// A silo configured before this existed has one target stored on its own,
/// and it is read as a list of one rather than migrated. Rewriting a
/// working connection to change nothing but its shape is a way to lose it
/// on the machine where the rewrite fails.
pub fn load_targets(silo_id: Uuid) -> Vec<BackupTarget> {
    ...
    // The single-target era. Still the shape most silos are in.
...
/// Replaces the list.
///
/// The first entry is also written to the single-target slot, so anything
/// still reading that (and an older build, if one is ever run against this
/// silo) sees a working connection rather than none.
...
mod tests {
    ...
    fn two_silos_keep_separate_configs_on_disk() {
        ...
        // configured last â€” which is how a family silo ends up replaying a
...
mod target_list_tests {
    ...
    #[test]
    fn a_silo_configured_before_this_reads_as_a_list_of_one() {
        // Migrating a working connection to change nothing but its shape is a
        // way to lose it on the machine where the rewrite fails, so the old
        // slot is read rather than rewritten.
        let scratch = Scratch::new();
