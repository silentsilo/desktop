...
pub fn set_cache_limit_bytes(app_data_dir: &Path, max_bytes: u64) -> Result<(), VaultError> {
    ...
    std::fs::write(app_data_dir.join(CACHE_SETTINGS_FILE), json)?;
    Ok(())
}
...
mod tests {
    ...
    /// The bug this guards against reached a user: the sidebar reported tens
    /// of megabytes still waiting to be backed up while the backup was in
    /// fact complete, because rows outlived the files they described.
    #[test]
