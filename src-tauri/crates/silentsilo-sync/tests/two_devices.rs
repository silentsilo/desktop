...
async fn a_second_sync_transfers_nothing_new() {
    ...
    // Records sitting exactly at the high-water mark *are* re-fetched: the
    // bound has to be inclusive because a Lamport value is not unique across
    // devices (see `fetch_ops_from`). Everything below it is skipped from the
    // listing alone, so the cost stays constant rather than growing with
    // history.
    assert!(
        second.fetched <= ops.len(),
        "an idle sync should refetch at most the boundary, got {}",
        second.fetched
    );
}
