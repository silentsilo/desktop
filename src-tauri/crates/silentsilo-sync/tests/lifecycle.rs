...
//! copy alone?**
//!
//! That question is the point. A target added after the first was once
//! offered nothing and reported itself up to date; every function involved
//! was correct on its own, and no test asked what the copy actually held.
//! Asking it here is what makes the class of bug visible.
//!
...
use silentsilo_sync::fetch_all_ops_above;
...
impl Silo {
    ...
    async fn sync(&mut self) {
        ...
        let kek_envelope = silentsilo_vault::wrap_kek_bytes(&self.session.kek, &self.dek).unwrap();

        for target in &self.targets {
            let owed = pending_ops_for(self.conn(), target.id).unwrap();
            // The app's own pass, not a copy of it. A harness that reproduced
            // these steps would go on passing while the real one changed
            // underneath it, which is how the delivery bug survived.
            let outcome = silentsilo_sync::push_everything_to(
                &silentsilo_sync::TargetPush {
                    id: target.id,
                    store: &target.store,
                    allows_delete: true,
                    owed: &owed,
                },
                &silentsilo_sync::SiloState {
                    ...
                    keys: None,
                    base: base.as_ref(),
                    vault_root: &self.session.paths.root,
                },
            )
            .await;
            assert!(
    ...
    async fn assert_every_copy_is_whole(&self, when: &str) {
        ...
        for target in &self.targets {
                ...
                .unwrap()
            {
                ...
                None => {
                    let records = fetch_all_ops_above(&target.store, &self.dek, 0)
                        .await
                        .unwrap();
    ...
    async fn assert_files_decrypt(&self, newcomer: &VaultSession, store: &FolderStore, when: &str) {
        ...
        for (file_id, expected) in &self.plaintext {
            ...
            silentsilo_sync::fetch_blob_to_file(store, entry.blob_id, &sealed)
                .await
                .unwrap();
...
/// ones that were not.
/// Twelve keeps this around two minutes, which is what a pass on every push
/// can afford. Raise it to a few hundred by hand after touching anything in
/// the delivery or compaction paths; that run takes an hour and is worth it
/// before a release.
const SEEDS: u64 = 12;
