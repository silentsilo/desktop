...
///
/// Used by the operations that remove something rather than add it. Deleting
/// from the primary target alone would leave the object readable on the
/// second one, which for a revoked key envelope means revocation that did
/// not happen. Targets that will not open are skipped: an unreachable one
/// cannot be deleted from, and the caller reports what it managed.
