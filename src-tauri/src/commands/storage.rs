...
use silentsilo_vault::clear_s3_config;
...
pub enum StoreConfigInput {
    ...
    /// The pairing waiting to be confirmed, rather than settings typed in.
    ///
    /// Carries nothing: the details are held in app state, because they
    /// include a secret access key and the frontend has no business seeing
    /// one. Setting a computer up from a backup can therefore offer
    /// SilentSilo's own storage without the browser flow ever handing
    /// credentials to the window.
    Hosted {},
    WebDav {
        url: String,
        username: String,
...
impl StoreConfigInput {
    /// Replaces a `Hosted` placeholder with the pairing that is waiting.
    ///
    /// Anything else is returned untouched, so a caller that accepts both can
    /// pass everything through this and stop caring which it got.
    pub(crate) fn resolved(self, app: &AppHandle) -> Result<Self, String> {
        match self {
            Self::Hosted {} => crate::commands::hosted::pending_input(app)
                .ok_or_else(|| "Connect to SilentSilo storage first, then continue.".to_string()),
            other => Ok(other),
        }
    }
    ...
    pub(crate) fn into_config(self, existing: Option<StoreConfig>) -> Result<StoreConfig, String> {
        match self {
            // Resolved by `resolved`, before this is reached. Getting here
            // means a caller forgot, and inventing an empty config would
            // point them at nothing rather than say so.
            Self::Hosted {} => Err("This storage has not been connected yet.".into()),
            Self::S3 {
                endpoint,
                region,
                bucket,
...
pub fn s3_disconnect(app: AppHandle) -> Result<(), String> {
    ...
    // precisely what the user asked it to stop.
    silentsilo_vault::save_targets(silo.id, &[]).map_err(|e| e.to_string())?;
    clear_s3_config(silo.id);
    Ok(())
}
...
pub struct BackupTargetView {
    ...
    pub archive: bool,
    /// Space bought from SilentSilo. The screen uses it to stop offering to
    /// sell a second allocation to an account that already has one.
    pub hosted: bool,
}
...
pub async fn backup_target_add(
    ...
    // `Hosted` means the pairing waiting in app state, which is how buying
    // space adds a second copy rather than replacing the first.
    let hosted = matches!(config, StoreConfigInput::Hosted {});
    let config = config.resolved(&app)?.into_config(None)?;
    config
        .open()
        .map_err(|e| e.to_string())?
        .check()
    ...
    targets.push(silentsilo_vault::BackupTarget {
        ...
        hosted,
        // Only bought space has one, and only the pairing that is still
        // waiting can supply it.
        usage_token: if hosted {
            crate::commands::hosted::pending_usage_token(&app)
        } else {
            None
        },
    });
