...
test("the grid and the list show the same silo", async ({ page }) => {
  ...
  await expect(page.getByRole("cell", { name: "15 Nov 2023, 12:00" }).first()).toBeVisible();

  await page.getByRole("button", { name: "Switch to grid view" }).click();
  await expect(page.getByText("896.5 MB · 15 Nov 2023")).toBeVisible();
});
...
test("the context menu offers the file's whole verb set", async ({ page }) => {
  ...
  for (const verb of ["Open", "Save a copy", "Rename", "Info", "Move to trash"]) {
...
test("sections render what the sidebar promises", async ({ page }) => {
  // Counted before it is opened: stepping into an unlocked silo reads it.
  await page.getByRole("button", { name: "Trash (4)", exact: true }).click();
  ...
  await expect(page.getByText("4 things to look at across 7 entries")).toBeVisible();
