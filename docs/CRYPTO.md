...
prompts are the OS's business. What does not work: U2F-only keys, which
have no `hmac-secret`, and passkeys on phones reached over the QR-code
flow, which do not offer the extension; both are refused at enrollment
...
| `ProcessSignaturePolicy` (Microsoft-signed images only) | A DLL dropped beside the executable being loaded into the process |

The remaining path worth naming is a **tampered binary**, which can wait for the next unlock and take the key. Nothing inside the program can detect that reliably. Signed releases and reproducible builds are the answer, so that a user can check the binary against the published source.
