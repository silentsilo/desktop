...
describe("analyseHealth", () => {
  ...
  /* Bought space. The threshold matters because it is the difference
     between a warning someone can act on and one that arrives too late. */
  describe("bought space", () => {
    const GB = 1024 ** 3;
    const usage = (usedGb: number) => ({
      ...HEALTHY_SILO,
      hostedUsage: { usedBytes: usedGb * GB, quotaBytes: 100 * GB },
    });

    it("says nothing below four fifths", () => {
      expect(ids(analyseHealth([], usage(79), NOW))).not.toContain("hosted-space");
    });

    it("speaks up at four fifths", () => {
      const found = analyseHealth([], usage(80), NOW).find((f) => f.id === "hosted-space");
      expect(found?.severity).toBe("medium");
      expect(found?.title).toContain("80%");
    });

    it("treats nearly full as serious", () => {
      const found = analyseHealth([], usage(96), NOW).find((f) => f.id === "hosted-space");
      expect(found?.severity).toBe("high");
    });

    it("says so plainly once there is no room left", () => {
      const found = analyseHealth([], usage(100), NOW).find((f) => f.id === "hosted-space");
      expect(found?.title).toBe("Your SilentSilo storage is full");
    });

    /* A silo on the user's own bucket has no allowance to run out of, and
       neither has one whose figure could not be read. */
    it("says nothing when there is no figure", () => {
      expect(ids(analyseHealth([], HEALTHY_SILO, NOW))).not.toContain("hosted-space");
    });

    it("says nothing when the allowance is unknown", () => {
      const silo = { ...HEALTHY_SILO, hostedUsage: { usedBytes: 5 * GB, quotaBytes: 0 } };
      expect(ids(analyseHealth([], silo, NOW))).not.toContain("hosted-space");
    });
  });

  describe("how the bought space is standing", () => {
    const standing = (hostedStatus: string | null): SiloHealth => ({
      ...HEALTHY_SILO,
      hostedStatus,
    });

    it("says nothing while the account is in good standing", () => {
      expect(ids(analyseHealth([], standing("active"), NOW))).toEqual([]);
      expect(ids(analyseHealth([], standing(null), NOW))).toEqual([]);
    });

    it("mentions a failed payment without alarming, because writes continue", () => {
      const found = analyseHealth([], standing("past_due"), NOW).find(
        (f) => f.id === "hosted-past-due",
      );
      expect(found?.severity).toBe("medium");
      expect(found?.detail).toContain("keep running");
    });

    it("ranks a stopped backup as something to fix today", () => {
      // Both of these mean nothing new is being written, which is the state
      // that looks like nothing until a restore is needed.
      for (const status of ["readonly", "cancelled"]) {
        const found = analyseHealth([], standing(status), NOW).find((f) =>
          f.id.startsWith("hosted-"),
        );
        expect(found?.severity).toBe("high");
        expect(found?.fix).toBe("backup");
      }
    });

    it("tells a full space apart from an unpaid one", () => {
      // Full while paid removes nothing; unpaid ends with the space gone,
      // and saying the wrong one is either a false alarm or a false calm.
      const full = analyseHealth([], standing("readonly"), NOW)[0];
      expect(full?.detail).toContain("nothing is removed while the plan is paid");

      const unpaid = analyseHealth([], standing("cancelled"), NOW)[0];
      expect(unpaid?.detail).toContain("removed after the notice");
    });
  });
