...
import { dropDuplicates } from "../../lib/passwordImport";
...
export function PasswordsPanel({
  ...
  /// over. Password and card number are secrets; an email address and a
  /// public key are exactly the parts meant to be given out.
  const copyPassword = useCallback(
    async (entry: PasswordEntry) => {
      switch (typeOf(entry)) {
        case "note":
          await copyPlain(entry.id, entry.notes);
          return;
        case "identity":
          await copyPlain(entry.id, entry.id_email || entry.id_full_name || "");
          return;
        case "ssh_key":
          await copyPlain(entry.id, entry.ssh_public_key ?? "");
          return;
        case "card":
          await copySecretField(entry, entry.id, cardDigits(entry));
          return;
        case "login":
          await copySecretField(entry, entry.id, entry.password);
      }
    },
    ...
    <div className="pw-view">
      ...
      <div className="view-toolbar">
        ...
        <button
          ...
          title="Import logins from a Bitwarden, LastPass, 1Password or Chrome CSV export"
        >
          <Upload size={15} />
          <span>Import</span>
        </button>
