...
import { Copy, ExternalLink, HardDrive, Laptop, Plus, Trash2, Truck, X } from "lucide-react";
...
export function CopiesPanel({ busy, fullCopy, onActivity }: Props) {
  ...
  /// Which way this second place is being set up. Null while the question is
  /// still on screen: pre-selecting the paid option would make "both are
  /// equal" a claim we did not mean.
  ...
  // An account holds one allocation, so offering to sell a second would be
  // offering something that cannot be bought.
  const hasHosted = list.some((t) => t.hosted);
  ...
  return (
    <div className="panel-section">
      ...
      {/* Only asked when there is something to ask. A silo already using its
          allocation has one way left to add a copy, and putting that behind a
          menu of one is a click that decides nothing. */}
      {adding && route === null && !hasHosted && (
        ...
        <div className="copies-add">
          <HostedStorageFlow
            ...
            onConnected={() => {
              setAdding(false);
              setRoute(null);
              setNote("SilentSilo storage added as another copy.");
              void refresh();
              onActivity();
            }}
      ...
      {adding && route === "own" ? (
        <div className="copies-add">
          {hasHosted && (
            <p className="hint">
              This silo already uses your SilentSilo storage, and an account holds one allocation,
              so a further copy goes somewhere you provide. The size of the one you have is changed
          ...
          <div className="actions">
            ...
            {/* "Back" is only true when there was a question to go back to.
                Skipping it lands here directly, so the way out is out. */}
            <button
              type="button"
              className="secondary"
              disabled={working}
              onClick={() => {
                setRoute(null);
                setError(null);
                if (hasHosted) setAdding(false);
              }}
            >
              <X size={15} />
              {hasHosted ? "Cancel" : "Back"}
            </button>
          </div>
        </div>
      ) : (
        !adding && (
          <div className="actions">
            <button
              ...
              onClick={() => {
                setAdding(true);
                // Straight to the form when the allocation is already spent:
                // there is no second option to weigh it against.
                if (hasHosted) setRoute("own");
              }}
            >
              <Plus size={15} />
              Add another place
