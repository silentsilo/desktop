...
/**
 * The same, in the units storage is sold in.
 *
 * Bought space is quoted decimally, because that is how the provider bills
 * and what the label on the plan claims. Printing a 100 GB allowance through
 * the binary formatter above gives "93.1 GB", which reads as either a
 * mistake or a swindle. Used only where the number came from a purchase;
 * a file on disk is still measured the other way.
 */
