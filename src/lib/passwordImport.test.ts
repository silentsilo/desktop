...
describe("applyImportCategory", () => {
  it("leaves the file's categories alone when asked to", () => {
    const list = [entry({ category: "Work" }), entry({ category: "Home" })];
    expect(applyImportCategory(list, { kind: "file" })).toBe(list);
  });

  it("refiles everything into the chosen category", () => {
    const list = [entry({ category: "Work" }), entry({ category: "Home" })];
    const out = applyImportCategory(list, { kind: "into", category: "Imported" });
    expect(out.map((e) => e.category)).toEqual(["Imported", "Imported"]);
    // The originals stay untouched; the caller may still cancel.
    expect(list[0].category).toBe("Work");
  });

  it("falls back to General when the chosen name is blank", () => {
    const out = applyImportCategory([entry({})], { kind: "into", category: "   " });
    expect(out[0].category).toBe("General");
  });

  it("does not change what the duplicate check sees", () => {
    const stored = entry({ service: "Same", category: "Old" });
    const refiled = applyImportCategory([entry({ service: "Same", category: "New" })], {
      kind: "into",
      category: "Imported",
    });
