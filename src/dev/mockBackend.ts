...
const handlers: Record<string, Handler> = {
  ...
  // Bought space. `?hosted` gives room to spare, `?fullspace` puts the
  // account over four fifths so the Health finding and the warning before
  // an import can both be seen. Neither is reachable in a preview
  // otherwise: it takes a purchase and a few hundred gigabytes.
  hosted_usage: () => {
    if (!flag("hosted") && !flag("fullspace")) return null;
    // Decimal, the unit the space is sold and billed in.
    const GB = 1000 ** 3;
    const quota = 250 * GB;
    const used = flag("fullspace") ? 231 * GB : 60 * GB;
    return {
      silo_bytes: used,
      account_bytes: used,
      quota_bytes: quota,
      status: "active",
      measured_at: new Date(Date.now() - 3 * 3600 * 1000).toISOString(),
    };
  },
  ...
  // `?hosted` returns bought space instead of a folder, which is the only
  // way to see the summary a paying customer gets: it needs a real purchase
  // and a real pairing to reach otherwise.
  s3_get_config: () => {
    if (flag("nobackup")) return null;
    if (flag("hosted")) {
      return {
        kind: "s3",
        bucket: "silentsilo-eu",
        prefix: "8f2a1c9b/3d7e5a04/",
        endpoint: "https://s3.eu-central-003.backblazeb2.com",
        region: "eu-central-003",
        access_key_id: "0031f2a9c4e7b60000000001",
      };
    }
    ...
    return [
      {
        ...
        retry_in: 0,
        archive: false,
        // ?hosted models a silo already using bought space, where the panel
        // must stop offering to sell a second allocation.
        hosted: flag("hosted"),
      },
      {
  ...
  // Hosted storage pairing. Walked through without a service: the first
  // poll answers "pending" so the waiting state can be looked at, and the
  // second approves, so the confirmation screen is reachable. That screen is
  // the security step of the whole flow and the one most worth staring at.
  hosted_pair_start: () => {
    hostedPolls = 0;
    return {
      user_code: "7K2M-QRTX",
      device_code: "mock-device-code",
      verification_url: "http://localhost:3100/pair",
      expires_in: 600,
      interval: 2,
    };
  },
  hosted_pair_poll: () => {
    hostedPolls += 1;
    if (hostedPolls < 2) return { status: "pending" };
    return {
      status: "approved",
      display: {
        ...
        label: "Silo 1",
        plan: "500 GB",
        region: "European Union (Amsterdam)",
        portalUrl: "http://localhost:3100/account",
      },
    };
  },
  hosted_pair_confirm: () => null,
  hosted_pair_cancel: () => null,
  hosted_portal_url: () => "http://localhost:3100/account",
